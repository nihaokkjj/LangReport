...
> 执行授权：用户于 2026-09-22 指示执行本任务；T0–T8 按依赖顺序推进，登录网关服务端合同保持独立。
...
T8.1–T8.3 已完成本轮切片：Snapshot controller 已接管列表/详情读取、版本排序、Abort/stale-response 防护、关闭清理和重试；页面仅保留打开门禁与 Modal 展示映射。21 个 Web unit、Web/全仓 typecheck、Web/全仓 build、Web/全仓 test、18 条桌面/移动 E2E、docs:check 和 `git diff --check` 已通过。T8 的 Evidence/Review 页面组合层与公共组件审计不在本轮标记完成。
