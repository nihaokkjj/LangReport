...
全仓 `typecheck/test/build` 与 `docs:check` 已通过；API Console 401 smoke、Snapshot 预览、Review comments、Plugin trace、Evidence canvas 和 Review composition 桌面/移动回归已通过；T8 跨 feature 公共组件审计、独立验证角色和真实 logout/cache 场景仍属于后续验收。
