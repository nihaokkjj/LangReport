...
- Query cache 的跨身份清理和真实 logout 过程仍缺少真实 Cookie Jar 浏览器场景：登录→读取 Project/Evidence→登出→确认旧 DOM、Query cache、local selection 和 watcher 均清理→重新登录。
- 当前 Web E2E 使用 route mock，不能替代真实同源 Cookie 的刷新恢复与登出后拒绝访问。
