# LangReport 文档导航

`docs/` 根目录只保留本导航。其余文档按用途和读者场景归类；产品术语仍以仓库根 [CONTEXT.md](../CONTEXT.md) 为准，Agent 工作纪律仍以根 [AGENTS.md](../AGENTS.md) 为准。

| 分类 | 内容 |
| --- | --- |
| [product/](./product/) | 当前产品规格、路线与验收边界 |
| [architecture/](./architecture/) | 系统架构与领域模型 |
| [agent/](./agent/) | Agent 启动、循环与执行约束 |
| [generation/](./generation/) | Generation Cycle、Model Gateway、Harness 与 LangGraph 设计 |
| [plugins/](./plugins/) | 平台内置 Plugin Manifest 规范 |
| [web/](./web/) | Web 端专项说明 |
| [operations/](./operations/) | 开发环境、部署与运维记录 |
| [testing/](./testing/) | 测试体系计划和后续测试治理记录 |
| [research/](./research/) | 市场、技术与选型研究 |
| [audits/](./audits/) | 路由、调试和实施验收证据 |
| [history/](./history/) | 已归档的阶段设计参考 |
| [adr/](./adr/) | 架构决策记录 |

## 优先入口

- [第一阶段产品规格](./product/phase1-consulting-report.md)
- [Agent 启动与 Loop 规范](./agent/agent-loop-spec.md)
- [系统架构](./architecture/architecture.md)
- [领域模型](./architecture/domain-model.md)
- [测试系统搭建计划](./testing/testing-system-plan.md)
- [T3 核心分支覆盖基线](./testing/coverage-baselines.md)
- [T4 隔离集成测试环境](./testing/integration-environment.md)
