...
第一阶段产品闭环是“Project → Brief → Data Snapshot → Generation Cycle → Evidence Block → Review”。本文件记录三层记忆的工程方案；第一阶段只启用与咨询项目指标口径、术语和视觉偏好确认有关的能力，不因此扩大产品范围：
...
- `packages/contracts`：增加 Memory scope/type/status、Candidate、Context、接受/拒绝/删除请求契约；
- `packages/domain`：增加候选和长期记忆状态机、记忆权限矩阵、key/value 指纹及冲突上下文构建；
- `packages/db`：增加 `memories`、`memory_candidates`、`conversation_memory_snapshots`、`memory_extraction_jobs`，以及 Generation Job/Chart Revision 记忆字段；迁移为 `0005`、`0006`；
- `packages/memory`：实现 Conversation Memory、候选提取任务、候选确认/拒绝、Project/Workspace 检索、冲突处理、软删除和审计；
- `apps/api`：接入对话快照、候选管理、长期记忆查询/删除和 Revision 记忆上下文接口；
- `apps/generation-worker`：轮询记忆提取任务，并在生成前检索已确认记忆；
- `apps/render-worker` / `packages/chart`：将生成时的记忆引用、版本和 hash 固化到 Chart Revision；
- `apps/web`：增加项目记忆面板、候选确认/拒绝、作用域选择、冲突提示和生成结果记忆追溯。

当前候选提取使用 `packages/memory` 中的确定性规则提取器，作为 Model Gateway 接入前的本地 MVP 实现；提取器已经通过 `MemoryExtractor` 接口隔离，后续可以替换为模型实现。已验证 `pnpm typecheck`、`pnpm build`、Domain/Memory 单元测试。由于本机 Docker daemon 无权限，尚未执行真实 PostgreSQL migration 和 API/Worker 端到端运行验证。
