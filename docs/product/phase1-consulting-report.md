...
第一阶段不把 Workspace 作为用户可见的产品层级。系统为每个认证用户自动维护一个私有内部 Workspace，用户直接创建、切换和管理自己的 Project；不提供 Workspace 切换、成员管理或跨用户共享入口。Workspace ID 仍作为后端权限、插件、记忆和对象存储的技术隔离键。
...
第一阶段可以随平台版本发布内置的声明式 Plugin Manifest，用于提供已经审核的模板、Theme、字段语义和追加校验规则。系统在用户私有内部 Workspace 中维护当前账号可用能力，用户从项目入口启用或禁用精确版本。Manifest 校验入口也只接受与内置目录内容哈希一致的版本；平台拒绝上传来源、任意 Manifest 和插件市场内容。现有 Manifest 的持久化、哈希和快照基础设施只服务于内置能力的可追溯性，并为后续阶段保留，不构成插件市场承诺。
