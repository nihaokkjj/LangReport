...
当前部署目标是：Vercel 托管 `apps/web`，ECS 托管 API、PostgreSQL 和 MinIO。
...
API 只绑定到 ECS 本机的 `127.0.0.1:4000`，PostgreSQL 和 MinIO 不映射到公网。后续应由 Nginx/Caddy 监听 80/443，并反向代理到 `127.0.0.1:4000`。
