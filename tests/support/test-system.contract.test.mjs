...
const migrationLedgerEntries = {
  ...
  "apps/api/src/message-generation.integration.test.ts":
    "apps/api/test/integration/message-generation.integration.test.ts",
  "apps/api/src/plugins.integration.test.ts": "apps/api/test/integration/plugins.integration.test.ts",
  "apps/generation-worker/src/worker.integration.test.ts":
...
test("all TypeScript tests live in an agreed package-owned directory", () => {
  ...
  assert.deepEqual(
    testFiles.filter((path) => !allowed.test(path)),
    [],
  );
});
...
test("default package tests include only unit tests and contain no skips", () => {
  const unitTestFiles = normalizedRepositoryPaths((path) =>
    /^(?:apps|packages)\/[^/]+\/test\/unit\/.+\.test\.ts$/.test(path),
  );
  assert.ok(unitTestFiles.length > 0);
  const skippedTests = unitTestFiles.filter((path) =>
    /(?:\btest|\bdescribe)\.skip\s*\(|\{\s*skip\s*:/.test(readFileSync(repositoryPath(path), "utf8")),
  );
...
test("the offline runner rejects configured external services before tests start", () => {
  ...
  assert.match(
    `${result.stdout}\n${result.stderr}`,
    /Offline test mode refuses configured external environment: DATABASE_URL/,
  );
});
...
test("the PR workflow installs a frozen lockfile and runs the offline quality gates", () => {
