...
import { db, chartRevisions, dataAssets, dataSnapshots, generationJobs, memoryExtractionJobs } from "@langreport/db";
...
async function processEditJob(jobId: string, job: typeof generationJobs.$inferSelect): Promise<void> {
  ...
  try {
    ...
    await setStatus(jobId, "transforming", {
      ...
      memoryContext: job.memoryContext ?? { conversation: null, project: [], workspace: [], conflicts: [] },
      validation,
