...
export const routeContracts: RouteContract[] = [
  ...
  contract("POST", "/api/v1/workspaces/:workspaceId/plugins/validate", "validatePluginManifest", ["Plugins"], "校验平台内置 Plugin Manifest", { 200: dto({ summary: objectResponse, validationReport: pluginReportDto }, ["summary", "validationReport"]) }, { request: pathRequest("/api/v1/workspaces/:workspaceId/plugins/validate", { body: zodJson(pluginManifestSchema) }) }),
  contract("POST", "/api/v1/workspaces/:workspaceId/plugins", "installPlugin", ["Plugins"], "安装平台内置 Plugin Manifest", { 201: dto({ installation: pluginInstallationDto, summary: pluginReportDto, reused: boolean(), auditEventId: auditEventIdDto }, ["installation", "summary", "reused", "auditEventId"]), 200: dto({ installation: pluginInstallationDto, summary: pluginReportDto, reused: boolean(), auditEventId: auditEventIdDto }, ["installation", "summary", "reused", "auditEventId"]) }, { request: pathRequest("/api/v1/workspaces/:workspaceId/plugins", { body: json({ manifest: objectResponse, source: { type: "string", enum: ["builtin"] }, idempotencyKey: string() }, ["manifest", "source", "idempotencyKey"]) }) }),
  ...
  contract("POST", "/api/v1/projects/:projectId/data-assets/upload", "uploadDataAsset", ["Data Assets"], "上传并解析数据文件", { 201: dto({ asset: assetDto }, ["asset"]) }, { request: pathRequest("/api/v1/projects/:projectId/data-assets/upload", { body: json({ file: { type: "string", format: "binary", description: "待解析的数据文件" }, conversationId: { type: "string", format: "uuid", description: "来源 Conversation" } }, ["file", "conversationId"]), consumes: ["multipart/form-data"] }), extraResponses: { 413: errorResponseSchema } }),
  ...
  contract("POST", "/api/v1/projects/:projectId/analysis-brief", "createAnalysisBrief", ["Analysis Brief"], "创建 Project 当前 Analysis Brief", { 201: dto({ brief: briefDto }, ["brief"]) }, { request: pathRequest("/api/v1/projects/:projectId/analysis-brief", { body: zodJson(createAnalysisBriefRequestSchema) }) }),
  contract("PATCH", "/api/v1/projects/:projectId/analysis-brief", "updateAnalysisBrief", ["Analysis Brief"], "编辑或确认 Project 当前 Analysis Brief", { 200: dto({ brief: briefDto }, ["brief"]) }, { request: pathRequest("/api/v1/projects/:projectId/analysis-brief", { body: zodJson(updateAnalysisBriefRequestSchema) }) }),
