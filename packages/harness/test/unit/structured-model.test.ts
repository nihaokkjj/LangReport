...
import { sendStructuredModelRequest, type FetchLike } from "../../src/structured-model.js";
...
test("does not depend on LangReport application packages", async () => {
  ...
  const source = await readFile(new URL("../../src/structured-model.ts", import.meta.url), "utf8");
