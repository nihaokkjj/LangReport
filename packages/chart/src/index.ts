...
export async function createInitialRevision(input: {
  ...
  return db.transaction(async (tx) => {
    ...
    const [revision] = await tx.insert(chartRevisions).values({
      ...
      analysisBriefSnapshot: input.analysisBriefSnapshot ?? {},
...
export async function transitionChartRevision(input: {
  ...
  return db.transaction(async (tx) => {
    ...
    if (input.nextStatus !== "archived") {
      await tx.update(evidenceBlocks).set({
...
export async function getProjectTheme(projectId: string, userId: string) {
  ...
  return theme
    ? { ...theme, themeRef: pluginThemeRefSchema.nullable().parse(theme.themeRef ?? null) }
    : { projectId, preset: "economist", themeRef: null, version: 1, config: {}, updatedBy: "system", updatedAt: new Date() };
}
